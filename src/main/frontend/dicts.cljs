...
(def dicts
  {:en {:tutorial/text (rc/inline "tutorial-en.md")
        ...
        :syncserver/add-repo-prompt-confirm "Add and install"
        :syncserver/add-repo-prompt "Sync notes with new vault"
        :syncserver/add-vault-prompt-confirm "Add and install"
        :syncserver/set-access-token "Set SyncServer personal access token"
        :syncserver/token-is-encrypted "The token will be encrypted and stored in the browser local storage"
        :syncserver/token-server "The server will never store it"
        :syncserver/create-personal-access-token "How to create a SyncServer personal access token?"
        :syncserver/push "Send To Vault now"
        :syncserver/push-failed "Send To Vault failed!"
        :syncserver/local-changes-synced "All local changes are synced!"
        :syncserver/pull "Get from Vault now"
        :syncserver/last-pull "Last got from vault at"
        :syncserver/version "Version"
        :syncserver/import-notes "Import your notes"
        :syncserver/import-notes-helper "You can import your notes from a vault on SyncServer."
        :syncserver/add-another-repo "Add another vault"
        :syncserver/re-index "Clone again and re-index the db"
        ...
        :add-step-shortcut-title "Click to add new step to latest journal"
        :loading "Loading"
        :cloning "Cloning"
        ...
        :login-syncserver "Login to Sync Server with Matrix ID"
        :login "Login"
        :go-to "Go to "
        :or "or"
        :download "Download"
  ...
  :ru {:tutorial/text (rc/inline "tutorial-en.md")
        :tutorial/dummy-notes (rc/inline "dummy-notes-en.md")
        :on-boarding/title "Hi, welcome to Logseq!"
        :on-boarding/sharing "sharing"
        :on-boarding/is-a " is a "
        ...
        :git/add-repo-prompt "Install Logseq on your repo"
        :git/add-repo-prompt-confirm "Add and install"
        :syncserver/add-vault-prompt-confirm "Добавить и установить"
        :syncserver/set-access-token "Задать персональный ключ доступа в сервер синхронизации"
        :syncserver/token-is-encrypted "Ключ доступа будет зашифрован и сохранён в локальном хранилище браузера"
        :syncserver/token-server "Сервер никогда не будет его хранить"
        :syncserver/create-personal-access-token "Как создать персональный ключ доступа в сервер синхронизации?"
        :syncserver/push "Отправить в сейф сейчас"
        :syncserver/push-failed "Отправка в сейф неуспешна!"
        :syncserver/local-changes-synced "Все локальные изменения синхронизованы!"
        :syncserver/pull "Получить из сейфа сейчас"
        :syncserver/last-pull "Получено из сейфа в последний раз:"
        :syncserver/version "Версия"
        :syncserver/import-notes "Импортировать ваши заметки"
        :syncserver/import-notes-helper "Вы можете импортировать ваши заметки из сейфа на сервере синхронизации"
        :syncserver/add-another-repo "Добавить новый сейф"
        :format/preferred-mode "What's your preferred mode?"
        :format/markdown "Markdown"
        :format/org-mode "Org Mode"
        :reference/linked "Linked Reference"
        ...
        :search (if config/publishing?
                  "Поиск"
                  "Найти страницу либо создать новую")
        :page-search "Поиск на этой странице"
        ...
        :export-markdown "Экспортировать как стандартный Markdown (без свойств блоков)"
        :export-opml "Экспортировать как OPML"
        :export-public-pages "Экспортировать общедоступные страницы"
        :export-json "Экспортировать как JSON"
        :export-edn "Экспортировать как EDN"
        :export-datascript-edn "Экспортировать datascript EDN"
        :convert-markdown "Convert Markdown headings to unordered lists (# -> -)"
        :all-graphs "All graphs"
        ...
        :import "Import"
        :join-community "Join the community"
        :login-syncserver "Войти в сервер синхронизации с помощью Matrix ID"
        :sponsor-us "Sponsor Us"
        :discord-title "Our discord group!"
        :sign-out "Выйти"
        :help-shortcut-title "Click to check shortcuts and other tips"
        ...
        :user/delete-account "Delete account"
        :user/delete-your-account "Delete your account"
        :user/delete-account-notice "Все страницы которые вы опубликовали на logseq.com будут удалены."
